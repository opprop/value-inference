...
in `cast_checker` clone, I've attached a `FooProject` as a demo of ReadChecker. You can run Read Checker on this foo project in two ways:
...
```bash
cast_checker/../checker-framework/checker/bin-devel/javac -processor cast.cast_checker -cp ReadChecker/bin:cast_checker/lib <your java files>
```
